...
if(!empty($_POST["classid1"])) 
{
 ...
 while($row=$stmt->fetch(PDO::FETCH_ASSOC))
 {?>
  <p> <?php echo htmlentities($row['SubjectName']); ?><input type="is_numeric"  name="marks[]" value="" class="form-control" required="" placeholder="Enter marks out of 70" autocomplete="off"></p>
  <p> <?php echo htmlentities($row['SubjectName']); ?><input type="is_numeric"  name="Insem[]" value="" class="form-control" required="" placeholder="Enter marks out of 30" autocomplete="off"></p>
<?php  }
}
}
